...
function walletPathExists(directory, filename, isIOS, walletManager) {
    ...
    if(isIOS)
        var path = electroneumAccountsDir + filename;
    else
...
function getApproximateBlockchainHeight(_date, _nettype){
    // time of electroneum birth 2014-04-18 10:49:53 (1397818193)
    var electroneumBirthTime = _nettype == "Mainnet" ? 1397818193 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    ...
    var secondsPerBlock;
    // before electroneum's birth
    if (requestedTime < electroneumBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < electroneumBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > electroneumBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - electroneumBirthTime)/secondsPerBlockV1);
